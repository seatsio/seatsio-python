...
class RetrieveMyAccountTest(SeatsioClientTest):
    ...
    def test(self):
        ...
        assert_that(account.settings.chartValidation.validateDuplicateLabels).is_equal_to("WARNING")
        assert_that(account.settings.chartValidation.validateObjectsWithoutCategories).is_equal_to("WARNING")
        assert_that(account.settings.chartValidation.validateUnlabeledObjects).is_equal_to("WARNING")
