...
class ExponentialBackoffTest(SeatsioClientTest):
    ...
    def test_aborts_eventually_if_server_keeps_returning_429_get(self):
        start = time.time()
        try:
            client = HttpClient(self.httpbin_url(), "aSecretKey", None, 5)
            client.url("/status/429").get()
            raise Exception("Should have failed")
        except RateLimitExceededException as e:
            assert_that(e.message).is_equal_to("Error while executing GET " + self.httpbin_url() + "/status/429")
    ...
    def test_aborts_eventually_if_server_keeps_returning_429_post(self):
        ...
        try:
            ...
            assert_that(e.message).is_equal_to("Error while executing POST " + self.httpbin_url() + "/status/429")
            wait_time = int(time.time() - start)
            assert_that(wait_time).is_between(10, 20)
    ...
    def test_aborts_eventually_if_server_keeps_returning_429_delete(self):
        ...
        try:
            ...
            assert_that(e.message).is_equal_to("Error while executing DELETE " + self.httpbin_url() + "/status/429")
            wait_time = int(time.time() - start)
            assert_that(wait_time).is_between(10, 20)
    ...
    def test_aborts_directly_if_server_returns_other_error_than_429(self):
        ...
        try:
            ...
            assert_that(e.message).is_equal_to("Error while executing GET " + self.httpbin_url() + "/status/400")
            wait_time = int(time.time() - start)
    ...
    def test_aborts_directly_if_server_returns_429_but_max_retries_0(self):
        start = time.time()
        try:
            client = HttpClient(self.httpbin_url(), "aSecretKey", None, 0)
            client.url("/status/429").get()
            raise Exception("Should have failed")
        except RateLimitExceededException as e:
            assert_that(e.message).is_equal_to("Error while executing GET " + self.httpbin_url() + "/status/429")
