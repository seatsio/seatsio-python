...
class EventReportsTest(SeatsioClientTest):
    ...
    def test_reportItemPropertiesForGA(self):
        ...
        assert_that(report_item.has_lift_up_armrests).is_none()
        assert_that(report_item.is_hearing_impaired).is_none()
        assert_that(report_item.is_semi_ambulatory_seat).is_none()
        assert_that(report_item.has_sign_language_interpretation).is_none()
        assert_that(report_item.is_plus_size).is_none()
