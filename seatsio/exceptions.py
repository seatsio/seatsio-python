...
class SeatsioException2(Exception):
    def __init__(self, request, response=None, cause=None):
        if (response is not None) and ("application/json" in response.headers["Content-Type"]):
            ...
            self.cause = cause
            super(SeatsioException2, self).__init__(self.__build_exception_message(request, response))
        else:
            self.messages = None
            self.requestId = None
            self.cause = cause
            super(SeatsioException2, self).__init__(
                "Error while executing " + request.httpMethod + " " + request.url + ". Cause: " + cause.message)
